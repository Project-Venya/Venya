...
impl Tacotron2 {
    ...
    pub fn load(path: impl AsRef<Path>) -> anyhow::Result<Self> {
        ...
        println!(
            "Loading Tacotron2 model from {:?}",
            path.as_ref().join("encoder.onnx").display()
        );

        let ort_path = std::env::var("ORT_DYLIB_PATH").expect("ORT_DYLIB_PATH not set");
        ...
        let encoder = Session::builder()?
            ...
            .with_context(|| {
                format!(
                    "converting encoder to runnable model from {}",
                    path.as_ref().join("encoder.onnx").display()
                )
            })?;
